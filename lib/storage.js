...
function Storage(options, cb) {
  ...
  if (options.email) {
    ...
    var pw = crypto.prepare_key(new Buffer(options.password))
    ...
    self.api.request({a: 'u', ssl: 0, ms: '-1', s: size, r: 0, e: 0},
      function(err, resp) {
        ...
        util.stream2cb(httpreq, function(err, hash) {
          ...
          File.getCipher(key).encrypt_cbc(at)
          ...
          self.api.request({
            ...
            n: [{
              h: hash.toString(),
              t:0 ,
              a: crypto.base64Addons(at.toString('base64')),
