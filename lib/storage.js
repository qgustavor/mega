...
class Storage extends EventEmitter {
  ...
  login (cb) {
    ...
    const pw = prepareKey(new Buffer(this.options.password))

    // after generating the AES key the password isn't needed anymore
    delete this.options.password

    let aes = new AES(pw)
    const uh = e64(aes.stringhash(new Buffer(this.email)))
  ...
  reload (force, cb) {
    ...
    this.api.request({a: 'f', c: 1}, (err, response) => {
      ...
      this.shareKeys = response.ok.reduce((shares, share) => {
        ...
        const auth = this.aes.encryptECB(new Buffer(handler + handler))
  ...
  _importFile (f) {
    ...
    if (!this.files[f.h]) {
      const fo = this.files[f.h] = new MutableFile(f, this)
