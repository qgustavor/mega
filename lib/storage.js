  ...
  constructor (options, cb) {
    ...
    if (options.email) {
      ...
      this.api.request({a: 'us', user: options.email, uh: uh}, (err, response) => {
        ...
        let rsaPrivk = Array(4)

        // decompose private key
        for (let i = 0; i < 4; i++) {
          const l = ((privk.charCodeAt(0) * 256 + privk.charCodeAt(1) + 7) >> 3) + 2
          rsaPrivk[i] = mpi2b(privk.substr(0, l))
          if (typeof rsaPrivk[i] === 'number') break
          privk = privk.substr(l)
        }

        let sid = new Buffer(b2s(RSAdecrypt(t, rsaPrivk[2], rsaPrivk[0], rsaPrivk[1], rsaPrivk[3])).substr(0, 43), 'binary')
        sid = crypto.e64(sid)

        self.api.sid = self.sid = sid
        self.RSAPrivateKey = rsaPrivk
