  ...
  constructor(options, cb) {
    ...
    if (options.email) {
      ...
      this.api.request({a: 'us', user: options.email, uh: uh}, (err, response) => {
        ...
        const t = rsa.mpi2b(crypto.formatKey(response.csid).toString('binary'));
        ...
        let sid = new Buffer(rsa.b2s(rsa.RSAdecrypt(t,rsa_privk[2],rsa_privk[0],rsa_privk[1],rsa_privk[3])).substr(0, 43), 'binary');
        sid = crypto.e64(sid)
