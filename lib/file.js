...
class File extends EventEmitter {
  ...
  static fromURL (opt) {
    ...
    if (url.path.match(/\/(file|folder)\//) !== null) {
      // new format
      const fileHandker = url.path.substring(
        url.path.lastIndexOf('/') + 1,
        url.path.length + 1
      )
      const fileKey = url.hash.substr(1)

      if ((fileHandker && !fileKey) || (!fileHandker && fileKey)) throw Error('Invalid URL: too few arguments')

      return new File({
        downloadId: fileHandker,
        key: fileKey,
