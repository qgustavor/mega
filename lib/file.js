...
class File extends EventEmitter {
  constructor (opt) {
    ...
    if (opt.api) {
      if (!(opt.api instanceof API)) throw Error('api must be a instance of API')
      this.api = opt.api
    } else {
      // Create a new API object on demand
      if (!API.notLoggedApi) {
        API.notLoggedApi = new API(false)
      }
      this.api = API.notLoggedApi
    }
  ...
  download (options, cb) {
    ...
    this.api.request(req, (err, response) => {
      ...
      const getChunk = () => {
        ...
        this.api.fetch(response.g + '/' + currentOffset + '-' + currentMax)
          .then(response => {
            handleMegaErrors(response)
