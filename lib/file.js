  ...
  loadAttributes (cb) {
    ...
    this.api.request(req, (err, response) => {
      ...
      if (this.directory) {
        ...
        // if it's hard to understand run the code below in console
        // [1, 2, 3, 4, 5, 6].map(e => ({n: e, t: e % 2})).sort((a,b) => b.t - a.t)
        nodes.sort((a, b) => {
          return b.t - a.t
        })
      ...
      function getChunk () {
        ...
        r.on('response', (resp) => {
          if (resp.statusCode === 200) {
            r.resume()
          } else if (resp.statusCode === 509) {
            stream.emit('error', Error('Bandwidth limit reached: ' + resp.headers['x-mega-time-left'] + ' minutes until it resets'))
          } else {
