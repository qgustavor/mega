...
class File extends EventEmitter {
  ...
  download (options, cb) {
    ...
    if (start % 16 !== 0) throw Error('For now is only possible to start downloading in multiples of 16 bytes.')
    ...
    // warning: not providing a key will return the encrypted stream
    const stream = this.key && !options.returnCiphertext
      ? megaDecrypt(this.key, {start})
      : new PassThrough()
    ...
    cs.request(req, (err, response) => {
      ...
      if (maxConnections === 1) {
        const r = requestModule(response.g + '/' + start + '-' + (end - 1))
