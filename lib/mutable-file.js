...
class MutableFile extends File {
  constructor (opt, storage) {
    ...
    if (opt.k) {
      let keyId = opt.k.split(':')[0]
      if (opt.u === keyId) keyId = opt.p
  ...
  mkdir (opt, cb) {
    ...
    if (!opt.key) opt.key = Buffer.from(secureRandom(32))
  ...
  _upload (opt, source, type, cb) {
    let encrypter = opt.uploadCiphertext
      ? through()
      : megaEncrypt(opt.key)
  ...
  _uploadWithSize (stream, size, source, pause, type, opt, cb) {
    const ssl = IS_BROWSER_BUILD && opt.forceHttps ? 2 : 0
    const request = type === 0
      ? {a: 'u', ssl, s: size, ms: 0, r: 0, e: 0, v: 2}
      : {a: 'ufa', ssl, s: size}

    if (opt.handle) {
      request.h = opt.handle
    }
    ...
    const handleChunk = (start, remaining) => {
      // Do not process chunk if it starts after the end of the file
      // or if the maximum number of connections were reached
      if (start >= size || activeConnections >= maxConnections) {
        return
      }

      activeConnections++
      const chunkSize = Math.min(currentChunkSize, size - start)
      const uploadBuffer = Buffer.alloc(chunkSize)
      ...
      let position = 0
      if (remaining) {
        remaining.copy(uploadBuffer)
        position = remaining.length
      }

      const dataHandler = (data) => {
        data.copy(uploadBuffer, position)
        position += data.length

        if (position >= chunkSize) {
          pause.pause()
          source.removeListener('data', dataHandler)

          const remaining = data.slice(0, position - chunkSize)
          sendChunk(uploadBuffer, start, chunkSize, remaining)
        }
      }
    ...
    const sendChunk = (chunk, position, chunkSize, remaining) => {
      ...
      streamToCb(httpreq, (err, hash) => {
        ...
        if (err || !hash || hash.length > 0) {
          ...
          handleChunk(nextPosition, remaining)
        }
      })
