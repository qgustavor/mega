...
class MutableFile extends File {
  ...
  _uploadWithSize (stream, size, source, pause, type, handle, cb) {
    // TODO: add setting to enforce HTTPS
    ...
    const initialChunkSize = type === 0 ? 128 * 1024 : size
    const chunkSizeIncrement = 128 * 1024
    ...
    const sendChunk = (chunk, position, chunkSize, remaining) => {
      ...
      streamToCb(httpreq, (err, hash) => {
        ...
        } else if (nextPosition < size && activeConnections < maxConnections) {
          handleChunk(nextPosition, remaining)
        }
      })

      if (nextPosition < size && activeConnections < maxConnections) {
        handleChunk(nextPosition, remaining)
      }
    }
