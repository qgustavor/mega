...
class MutableFile extends File {
  ...
  _uploadWithSize (stream, size, source, pause, type, opt, cb) {
    ...
    const handleChunk = () => {
      const chunkSize = Math.min(currentChunkSize, size - position)
      ...
      const dataHandler = (data) => {
        ...
        if (chunkPos >= chunkSize - 1) {
          ...
          remaining = data.slice(0, chunkPos - chunkSize)
          sendChunk(uploadBuffer)
        }
      }
    ...
    const sendChunk = (chunk) => {
      ...
      const reachedEnd = position >= size - 1

      streamToCb(httpreq, (err, hash) => {
        if (err || !hash || hash.length > 0) {
          cb(err, type, hash, source)
        } else if (!reachedEnd && !isReading) {
          handleChunk()
        }
      })

      if (!reachedEnd && !isReading && activeConnections < maxConnections) {
        handleChunk()
      }
    }
