...
import AbortController from './abort-controller-polyfill.cjs'
...
class File extends EventEmitter {
  ...
  download (options, cb) {
    ...
    this.api.request(req, (err, response) => {
      ...
      let stopped = false
      let appendStream
      const combined = new MultiStream(cb => {
        appendStream = cb
        if (stopped) cb(null, null)
      })
      ...
      const getChunk = () => {
        ...
        if (chunkMax < chunkOffset) {
          stopped = true
          return
        }

        const writeStream = new PassThrough()
        appendStream(null, writeStream)
