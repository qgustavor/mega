...
import { detectSize, streamToCb, pauseStream } from './util.mjs'
...
class MutableFile extends File {
  ...
  upload (opt, source, cb) {
    ...
    if (!opt.size && !(source && typeof source.pipe !== 'function' && source.length) && !opt.allowUploadBuffering) {
  ...
  _upload (opt, source, type, cb) {
    ...
    const pause = pauseStream(true)
    let stream = pumpify(pause, encrypter)
    ...
    if (size) {
      ...
      stream = pumpify(detectSize(size => {
        this._uploadWithSize(stream, size, encrypter, pause, type, opt, cb)
      }), stream)
    }
